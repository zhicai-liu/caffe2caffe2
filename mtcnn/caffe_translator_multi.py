...
##################################################################################
# Title : Caffe Translator
# Info : Translate Caffe model to Caffe2 model
# Forked from module [caffe2.python.caffe_translator] @ package [caffe_translator]
#
# Version 1.0
# Log : Expand usage to multi-inputs / multi-outputs
# Modifier : Jay Liu
# Date : 2019-08-09
# reference : https://blog.csdn.net/Jkwwwwwwwwww/article/details/88735621
##################################################################################
...
if __name__ == '__main__':
    ...
    ############################## Added by [Jay Liu] ##############################
    parser.add_argument("--input_num", help = "Amount of input(s), default 1", type = int,
                        default = 1)
    parser.add_argument("--output_num", help = "Amount of output(s), default 1", type = int,
                        default = 1)
    ############################## Added by [Jay Liu] ##############################
    parser.add_argument("--remove_legacy_pad", help = "Remove legacy pad (Only works for nets with one input blob), default False", action = "store_true",
                        default = False)
    parser.add_argument("--input_dims", help = "Dimension of input blob, default None", nargs = '+', type = int,
                        default = [])
    ...
    ############################## Modified by [Jay Liu] ##############################
    ## [input_num] input and [output_num] output
    input_num = args.input_num
    output_num = args.output_num

    external_input = [net.op[i].input[0] for i in range(0, input_num)]
    external_output = [net.op[o].output[0] for o in range(-1, -output_num-1, -1)]
    print (external_input)
    print (external_output)

    for e_input in external_input:
        net.external_input.extend([e_input])
    net.external_input.extend([param.name for param in pretrained_params.protos])
    for e_output in external_output:
        net.external_output.extend([e_output])
